...
class AddServiceController extends _$AddServiceController {
  ...
  Future<ServiceModel>addService({required name,required category,required description,required price,required imageUrls})async{
    try{
      final response = await ServicesApiServices().addService(name: name, category: category, description: description, price: price, imageUrls: imageUrls);
      return response;
    }catch(e){
      throw Exception(e);
    }
  ...
  Future<ServiceModel>updateService({required id,required name,required category,required description,required price,required imageUrls})async{
    try{
      final response = await ServicesApiServices().updateService(id: id, name: name, category: category, description: description, price: price, imageUrls: imageUrls);
      return response;
    }catch(e){
      throw Exception(e);
    }
